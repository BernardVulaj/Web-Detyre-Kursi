$(document).ready(function() {
    ...
    toggleProfileSections({ user: false, list: false, admin: false, menueadmin: false, car: false });
    get_user_main();
});
...
document.getElementById("fshiButton").addEventListener("click", function(){
    ...
    if (carId) {
    // Use fetch to send data with POST method
    fetch('deleteCar.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json', // Set the request content type
        },
        body: JSON.stringify({ car_id: carId }) // Send the car_id in the body
    })
    .then(response => response.json()) // Assuming the PHP file returns a JSON response
    .then(data => {
